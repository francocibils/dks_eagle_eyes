...
def process_ee_investment(df_raw):

    df_raw = df_raw[(df_raw['Consolidado'] == 'EAGLE EYES') & (df_raw['Proyecto'] == 'INBOUND TV')]
    df = df_raw.copy(deep = True)
    df['Fecha'] = pd.to_datetime(df['Fecha'])
    df = df.groupby(['Fecha', 'Proyecto'])[['Inversion']].sum()
    df = df.reset_index().drop(['Proyecto'], axis = 1)
